...
const DB_NAME: string = '__lifecheck';
...
export class Sql {
    ...
    _tryInit() {
        this.query('CREATE TABLE IF NOT EXISTS lifecheck (key text primary key, value text)').catch(err => {
    ...
    get(key: string): Promise<any> {
        return this.query('select key, value from lifecheck where key = ? limit 1', [key]).then(data => {
